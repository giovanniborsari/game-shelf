    ...
    <Link href={`/games/${game_id}`} >
        <div className = "w-2xl bg-transparent border-2 border-emerald-400 rounded-lg p-2 flex">
            ...
            <div className="flex flex-col items-start ml-3">
              <h1 className = "text-xl mt-1 font-mono text-white truncate max-w-110">{game_title}</h1>
              <p className="text-sm text-gray-300 mt-auto truncate max-w-110">{game_genre ?? " "}</p>
              <p className="text-sm text-gray-300 mt-0.5 truncate max-w-110">{game_platforms ?? " "}</p>
            </div>
